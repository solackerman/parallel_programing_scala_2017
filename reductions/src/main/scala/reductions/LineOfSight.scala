...
object LineOfSight {
  ...
  def angle(input: Array[Float], i: Int): Float =
    if (i == 0) 0
    else input(i) / i
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    ...
    for (i <- 1 until input.length)
      output(i) = max(angle(input, i), output(i-1))
  }
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    var ma = angle(input, from)
    for(i <- from + 1 until until) ma = max(angle(input, i), ma)
    ma
  }
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float],
    ...
    var ma = startingAngle
    for(i <- from until until) {
      ma = max(angle(input, i), ma)
      output(i) = ma
    }
  }
